...
public class GameController : MonoBehaviour {
    ...
    public Text scoreTxt, nowScoreText;
    ...
    private void Start()
    {
        ...
        scoreTxt.text = "<size=80><color=#E06156>best:</color></size>" + PlayerPrefs.GetInt("score");
        nowScoreText.text = "now:0";
    ...
    void SpawnPositions()
    {
        ...
        if (IsPositionEmpthy(new Vector3(nowCube.x + 1, nowCube.y, nowCube.z)) && nowCube.x + 1 != cubeToPlace.position.x)
            positions.Add(new Vector3(nowCube.x + 1, nowCube.y, nowCube.z));
        if (IsPositionEmpthy(new Vector3(nowCube.x - 1, nowCube.y, nowCube.z)) && nowCube.x - 1 != cubeToPlace.position.x)
            positions.Add(new Vector3(nowCube.x - 1, nowCube.y, nowCube.z));
        if (IsPositionEmpthy(new Vector3(nowCube.x, nowCube.y + 1, nowCube.z)) && nowCube.y + 1 != cubeToPlace.position.y)
            positions.Add(new Vector3(nowCube.x, nowCube.y + 1, nowCube.z));
        if (IsPositionEmpthy(new Vector3(nowCube.x, nowCube.y - 1, nowCube.z)) && nowCube.y - 1 != cubeToPlace.position.y)
            positions.Add(new Vector3(nowCube.x, nowCube.y - 1, nowCube.z));
        if (IsPositionEmpthy(new Vector3(nowCube.x , nowCube.y, nowCube.z + 1)) && nowCube.z + 1 != cubeToPlace.position.z)
    ...
    private bool IsPositionEmpthy(Vector3 targetPos)
    {
        if (targetPos.y == 0)
            return false;
